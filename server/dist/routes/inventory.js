...
// All inventory routes will be protected
router.use(auth_1.authMiddleware);
// Get inventory items with pagination and filters
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 10, search, category, status, condition, location } = req.query;
...
router.post('/', async (req, res) => {
    try {
        const { name, description, category, quantity = 1, condition = 'GOOD', location, purchaseDate, purchaseCost, currentValue, status = 'AVAILABLE', notes } = req.body;
...
router.put('/:id', async (req, res) => {
    try {
        const { name, description, category, quantity, condition, location, purchaseDate, purchaseCost, currentValue, status, notes } = req.body;
...
router.get('/stats/overview', async (req, res) => {
    try {
        const [totalItems, available, checkedOut, maintenance, retired, lost, excellent, good, fair, poor, damaged, totalValue] = await Promise.all([
