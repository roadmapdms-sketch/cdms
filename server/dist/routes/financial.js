...
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 10, memberId, type, paymentMethod, status, dateFrom, dateTo } = req.query;
...
router.post('/', async (req, res) => {
    try {
        const { memberId, type, amount, paymentMethod, description, reference, date, status = 'RECORDED' } = req.body;
        ...
        const financialRecord = await prisma.financialRecord.create({
            data: {
                memberId,
                userId: req.userId, // Get user from auth middleware
                type,
...
router.get('/stats/overview', async (req, res) => {
    try {
        ...
        const [totalRecords, totalIncome, totalExpenses, tithes, offerings, donations, fees] = await Promise.all([
            prisma.financialRecord.count({ where }),
...
router.get('/stats/monthly', async (req, res) => {
    try {
        ...
        const monthlyData = await Promise.all(Array.from({ length: 12 }, (_, month) => {
...
router.get('/stats/top-donors', async (req, res) => {
    try {
        ...
        const donorsWithDetails = await Promise.all(topDonors.map(async (donor) => {
            if (!donor.memberId)
                return null;
            const member = await prisma.member.findUnique({
...
router.post('/reports/generate', async (req, res) => {
    try {
        ...
        if (format === 'CSV') {
            ...
            const csvData = records.map(record => `${record.date.toISOString()},${record.type},${record.member ? `${record.member.firstName} ${record.member.lastName}` : 'N/A'},${record.amount},${record.paymentMethod || 'N/A'},"${record.description || ''}",${record.status}`).join('\n');
