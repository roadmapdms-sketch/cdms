...
// All vendor routes will be protected
  ...
  try {
    ...
    if (search) {
      where.OR = [
        ...
        { services: { contains: search as string, mode: 'insensitive' } }
      ];
    }
    ...
    if (services) {
      where.services = { contains: services as string, mode: 'insensitive' };
    }
...
router.get('/services/search', async (req, res) => {
  try {
    ...
    const vendors = await prisma.vendor.findMany({
      where: {
        isActive: true,
        services: { contains: service as string }
      },
      orderBy: { name: 'asc' }
    });
